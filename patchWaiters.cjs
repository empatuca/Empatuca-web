...
const regex = /\{product\.prices\.estandar !== undefined && \([\s\S]*?\}\)/m;
                    ...
                    product.variants.map((variant) => (
                      <div key={variant.id} className="flex items-center justify-between border-t border-dashed pt-2">
                        ...
                        <div className="flex items-center gap-3 shrink-0">
                          <Button variant="outline" size="icon" className="h-8 w-8 rounded-full bg-gray-100 text-gray-700 border-none" onClick={() => updateQuantity(\`\${product.id}-estandar-\${variant.id}\`, \`\${product.name} (\${variant.name})\`, "Unidad", product.prices.estandar || 0, -1)}>
                          ...
                          <Button variant="outline" size="icon" className="h-8 w-8 rounded-full bg-[#fac124] text-black border-none" onClick={() => updateQuantity(\`\${product.id}-estandar-\${variant.id}\`, \`\${product.name} (\${variant.name})\`, "Unidad", product.prices.estandar || 0, 1)}>
