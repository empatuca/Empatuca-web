...
// 3. FETCH HOOK
const fetchTarget = `      fetchOrders();

      const channel = supabase`;
const fetchReplacement = `      fetchOrders();
...
// 4. UI WRAPPER & LOGIC (before `return (`)
const logicTarget = `  return (
    <div className="min-h-screen bg-gray-50 text-gray-900">`;

const logicReplacement = `  const handleAddGasto = async (e: React.FormEvent) => {
    e.preventDefault();
...
// 6. GASTOS UI (replace the closing div of "container mx-auto" with the end of tab 1 + start of tab 2 + end of tab 2)
const endOfIngresosTarget = `      </div>
